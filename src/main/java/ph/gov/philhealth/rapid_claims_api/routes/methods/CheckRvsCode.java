 ...
 */
@RequestScoped
public class CheckRvsCode {
    @EJB
    private Utility utility;
    public RapidResult checkRvs(DataSource dataSource, String admission_code, String admission_date) {
        RapidResult result = this.utility.icdResult();
        
        try (Connection conn = dataSource.getConnection()) {
             
            String sql = "{ ? = call UCPS.ADMISSION_LOGBOOK_PKG.get_adm_rvs_amount (? , ? , ? , ?)}";
        ...
        Logger.getLogger(RapidService.class.getName()).log(Level.SEVERE, "SQLException CheckRvsCode checkRvs", e);
        }
